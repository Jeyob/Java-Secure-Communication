...
public class server_keyAgreement extends StateContext {
	...
	private final class AWATING_DH_PARAMETERS implements State {
		...
		public boolean doPhase() {
			...
			System.out.println("Server keyexchange: "+setup.getKeyExchangeAlgorithm());
			
			if(setup.getKeyExchangeAlgorithm().matches("DH[0-9]*")) { /* if normal diffie hellman */
			...
			}else if(setup.getKeyExchangeAlgorithm().matches("ECDH[0-9]*")) {
	...
	private final class AWAIT_CLIENT_PUBLICPART implements State {
		...
		public boolean doPhase() {
			...
			byte[] clientPublicPart = public_part.get(0);
			 
			
			mutualSecret = keyagreement.generateMutulSecret(keyagreement, clientPublicPart);
			HelperMetods.print2console("Mutual secret created");
			HelperMetods.printByteArray(mutualSecret);
			
			try {

				byte[] digest_data = createDigest(keyagreement.getPublicKey().getEncoded(),clientPublicPart, setup.getRecipient_name().getBytes());
	...
	private final class AWATING_CLIENT_SIGNED implements State {
		...
		public boolean doPhase() {
			...
			byte[] signedDigest = signed_data.get(0);
			byte[] parameters = signed_data.get(1);
			byte[] sessionANDcounter = signed_data.get(2);
			
			try {
				
				byte[] decrypted_data = publicCrypto.decrypter(signedDigest, setup.getOtherPublicKey(), setup.getPublicKeyAlgorithm(), RSA.NO_MODE, setup.getPublicPaddingAlgorithm());
...
//					}
					...
					dCipher.fromDHinit(sCipher.DECRYPT, mutualSecret , setup.getHashAlgorithm(), parameters);
					
					List<byte[]> sessionspecific =	HelperMetods.unfoldMessage(dCipher.doFinal(sessionANDcounter)); /* (0) sessionkey (1) counter */
					
					System.out.println("Sessionkey");
					HelperMetods.printByteArray(sessionspecific.get(0));

//					SecretKeySpec secretKeySpec = new SecretKeySpec(sessionspecific.get(0), setup.getSymmetricKeyAlgorithm());
					
					setup.counter = new BigInteger(sessionspecific.get(1)); //get counter
					
					System.out.println("Inserted counter "+setup.counter.longValue());
					
					setup.setSessionKey(sessionspecific.get(0)); /* sessionkey stored as raw bytes */
...
//					write2Partie(eCipher.getParameters(),eCipher.doFinal(Constants.KEY_EXCHANGE_SUCCESS.getBytes()));
					...
					write2Partie(Constants.KEY_EXCHANGE_SUCCESS.getBytes());
